...
exports.submit = async (req, res) => {
  ...
  [err, result] = await to(
    db.query(`SELECT answer from questions WHERE qno = ?`, [req.user.current])
  );
  if (err) return res.sendError(err);
  ans = result[0].answer;
  //console.log(ans);
  ...
  if (match) {
    [err, result] = await to(
      db.query(`UPDATE users SET current = current+1 WHERE id = ?`, [
        req.user.id
      ])
    );
    if (err) return res.sendError(err);
    ...
    [err, result] = await to(
      db.query(`SELECT qno,body,hint FROM questions WHERE qno = ?`, [
        req.user.current
      ])
    );
    ob = result[0];
